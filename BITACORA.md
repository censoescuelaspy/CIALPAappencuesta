...
## Impresion solo mapa y reconfirmacion de padron - 2026-07-02
...
### Objetivo de la intervencion
- Permitir imprimir solamente el mapa nacional, sin el resto de la hoja del atlas.
- Reconfirmar la cifra oficial consultada del padron.

### Acciones realizadas
- Se agrego el boton `Imprimir solo mapa` en el Atlas.
- El boton solo se habilita en la vista `Mapa nacional`.
- Se agrego una hoja de impresion dedicada para el coropletico nacional, sin tabla lateral ni texto extendido.
- Se reconfirmo la cifra del padron por dos vias independientes: exportacion CSV oficial y `diagnosticoPadron` del backend publico.
...
### Resultados verificados
- La exportacion CSV oficial sigue devolviendo `5463` lineas totales, es decir `5462` filas de escuelas mas encabezado.
- El endpoint publico `diagnosticoPadron` sigue devolviendo `total:5462`.
- `node --check assets/js/department-atlas.js` paso sin errores.

### Recomendaciones
- Tomar `5462` como total oficial vigente hasta que cambie la hoja `listado_ini` o el backend publico reporte otro universo.
