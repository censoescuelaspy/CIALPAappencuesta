...
- El mapa incorpora el acceso directo `Solo muestra (86)` y lo mantiene sincronizado con el filtro lateral.
- El cache del padrón queda separado por versión del marco MEC para no reutilizar la muestra anterior después de una actualización.
- Se elimina la inferencia que podía marcar como piloto todas las escuelas de un listado operativo pequeño.
- Si la muestra no coincide con las `86` escuelas esperadas, la app fuerza una recarga del padrón en línea y comunica el resultado.
- La prueba funcional ahora verifica que el filtro reduzca realmente el mapa y sus conteos.
...
- El Atlas departamental pasa a dos paneles en escritorio: mapa amplio a la izquierda y detalle/tabla sortable a la derecha.
- La tabla mantiene encabezados fijos y ordenables sin comprimir la visualizacion del mapa.
- En pantallas chicas el Atlas conserva una sola columna para mantener compatibilidad movil.

## Novedades v2.6.195

- El Atlas separa explicitamente el mapa de la tabla para evitar solapamientos visuales.
- Los encabezados de la tabla de escuelas ahora permiten ordenar por codigo, escuela, distrito/localidad, zona, estado, mapa y asignacion.
- La cabecera de tabla queda fija dentro del scroll y muestra el sentido del ordenamiento.
